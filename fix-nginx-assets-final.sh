...
# Final fix for asset 404 - root directive is interfering

set -e

echo "🔧 Final fix for asset 404..."
...
# The issue: root directive is being used instead of alias
# Solution: Ensure assets location comes before root, or use more specific matching
...
if (hopeApp) {
    ...
    // API FIRST (before root location)
...
// Ojoto Union - MUST come after root app but with specific locations FIRST
...
if (ojotoApp) {
    ...
    // Assets SECOND (very specific, before general location)
    ...
    // Main location for HTML (least specific, comes last)
    nginxConfig += '    location ' + frontendPath + '/ {\n';
...
console.log('Config regenerated with correct location order');
NODE_EOF
...
if sudo nginx -t 2>&1; then
    ...
    if [ "$HTTP_CODE" = "200" ]; then
        ...
        echo ""
        echo "Current config:"
        sudo cat /etc/nginx/conf.d/multi-app.conf | grep -B 2 -A 5 "location.*assets"
    fi
else
